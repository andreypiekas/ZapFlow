...
curl http://SEU_IP_SERVIDOR:3001/
...
2. Confirme:
   - ✅ **Enabled**: ON (verde)
   - ✅ **URL**: `http://SEU_IP_SERVIDOR:3001/api/webhook/evolution`
...
```bash
# Parar instância
curl -X DELETE http://SEU_IP_SERVIDOR:8080/instance/delete/piekas \
  -H "apikey: B8349283-F143-429D-B6C2-9386E8016558"

# Aguardar alguns segundos

# Criar/reiniciar instância (se necessário)
curl -X POST http://SEU_IP_SERVIDOR:8080/instance/create \
...
```bash
# Teste HTTP básico
curl -v http://SEU_IP_SERVIDOR:3001/

# Teste o endpoint de webhook diretamente
curl -v -X POST http://SEU_IP_SERVIDOR:3001/api/webhook/evolution \
  -H "Content-Type: application/json" \
  -d '{"test": "ok"}'

# Verificar se há firewall bloqueando
telnet SEU_IP_SERVIDOR 3001
...
- Se a Evolution API está em `SEU_IP_SERVIDOR:8080`
- E o backend está em `SEU_IP_SERVIDOR:3001`
- A URL deve ser: `http://SEU_IP_SERVIDOR:3001/api/webhook/evolution`
...
**USE:**
- ✅ IP real da VM/servidor (`SEU_IP_SERVIDOR`)
...
- URL base: `http://SEU_IP_SERVIDOR:3001/api/webhook/evolution`
- URL real usada: `http://SEU_IP_SERVIDOR:3001/api/webhook/evolution/MESSAGES_UPSERT`
...
```bash
# Ver versão
curl http://SEU_IP_SERVIDOR:8080/
```
...
curl -X POST http://SEU_IP_SERVIDOR:3001/api/webhook/evolution \
  -H "Content-Type: application/json" \
  -d '{
...
- [ ] Backend está acessível (`curl http://SEU_IP_SERVIDOR:3001/` funciona)
