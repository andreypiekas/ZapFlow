...
O erro `404 Not Found` ao acessar `http://192.168.101.234:8080/chat/ZapFlow` indica que esse endpoint **não existe** na Evolution API.
...
1. **Socket.IO** (mais comum):
   ```
   ws://192.168.101.234:8080/socket.io/?EIO=4&transport=websocket&instance=ZapFlow
   ```

2. **WebSocket direto** (se configurado):
   ```
   ws://192.168.101.234:8080/ws/ZapFlow
   ```

3. **WebSocket com autenticação**:
   ```
   ws://192.168.101.234:8080/socket.io/?instance=ZapFlow
   ```
...
```javascript
// Teste 1: Socket.IO
const ws1 = new WebSocket('ws://192.168.101.234:8080/socket.io/?EIO=4&transport=websocket&instance=ZapFlow');
ws1.onopen = () => console.log('✅ Socket.IO conectado');
ws1.onerror = (e) => console.error('❌ Socket.IO erro:', e);

// Teste 2: WebSocket direto
const ws2 = new WebSocket('ws://192.168.101.234:8080/ws/ZapFlow');
